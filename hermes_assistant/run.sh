...
if bashio::var.true "${ENABLE_TERMINAL}"; then
    ...
    /usr/local/bin/ttyd "${TTYD_ARGS[@]}" \
        bash -l -c "
            ...
            echo '    hermes setup     interactive wizard (Anthropic,'
            echo '                     OpenAI, Google, local models, …)'
            echo '    hermes model     pick provider + model'
            echo '    hermes update    pull latest agent'
            echo '    hermes --help    all commands'
            echo ''
            echo '  Claude Max subscription OAuth (optional):'
